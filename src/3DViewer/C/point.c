...
static void rotate_xp(point_t* point, const point_t* r_center,
                      const double angle) {
  double r_cos = cos(to_radians(angle));
  double r_sin = sin(to_radians(angle));
  double y = point->y;

  point->y = (point->y - r_center->y) * r_cos +
             (point->z - r_center->z) * r_sin + r_center->y;
  point->z = -(y - r_center->y) * r_sin + (point->z - r_center->z) * r_cos +
             r_center->z;
}

static void rotate_yp(point_t* point, const point_t* r_center,
                      const double angle) {
  double r_cos = cos(to_radians(angle));
  double r_sin = sin(to_radians(angle));
  double x = point->x;

  point->x = (point->x - r_center->x) * r_cos +
             (point->z - r_center->z) * r_sin + r_center->x;
  point->z = -(x - r_center->x) * r_sin + (point->z - r_center->z) * r_cos +
             r_center->z;
}

static void rotate_zp(point_t* point, const point_t* r_center,
                      const double angle) {
  double r_cos = cos(to_radians(angle));
  double r_sin = sin(to_radians(angle));
  double x = point->x;
  ...
  point->x = (point->x - r_center->x) * r_cos +
             (point->y - r_center->y) * r_sin + r_center->x;
  point->y = -(x - r_center->x) * r_sin + (point->y - r_center->y) * r_cos +
             r_center->y;
}
...
void scale_point(point_t* point, const point_t* s_center,
                 const scale_t* scale) {
...
my_error_t save_point(const point_t* point, FILE* fout) {
  int rc = fprintf(fout, "v %lf %lf %lf\n", point->x, -point->y, point->z);
